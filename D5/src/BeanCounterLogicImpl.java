...
public class BeanCounterLogicImpl implements BeanCounterLogic {
	...
	// number of slots. DO NOT TOUCH
	private int[] slots; 
	// the place where the beans will fall
	private int beanCount;
	// number of beans. DO not touch
	private int remainingBeanCount;
	// numbers of beans left after each iteration of advance step
	private Bean[] beansArr; 
	// holds all the beans. Do not mess with this ever.

	private ArrayList<Bean> inFlightBeans; // not used for ypos
	private ArrayList<Bean> beansList; // tells us the ypos of the beans
	// private Bean[] inFlightBeansArr;
	...
	BeanCounterLogicImpl(int slotCount) {
		...
		this.inFlightBeans = new ArrayList<Bean>();
		this.beansList = new ArrayList<Bean>();
	...
	public int getInFlightBeanXPos(int yPos) {
		...
		if (yPos > beansList.size()) {
			return NO_BEAN_IN_YPOS;
		}

		Bean b = beansList.get(yPos);
		if (b == null) {
			return NO_BEAN_IN_YPOS;
		}
		return b.getXPos();
	...
	public void reset(Bean[] beans) {
		// TODO: Implement
		this.beansArr = new Bean[beans.length];
		for (int i = 0; i < beans.length; i++) {
			this.beansArr[i] = beans[i];
		}
		// this.beansArr = beans;
		this.beanCount = beans.length;
		this.remainingBeanCount = this.beanCount;
		this.slots = new int[slotCount];
		this.inFlightBeans = new ArrayList<Bean>();
		this.beansList = new ArrayList<Bean>();
		for (int i = 0; i < slotCount; i++) {
		...
		// starting the machine with a bean if there is one
		if (remainingBeanCount > 0) {
			Bean firstBean = beans[remainingBeanCount - 1];
			beansList.add(0, firstBean);
	...
	public void repeat() {
		...
		slots = new int[slotCount]; // cleaning the slots
		...
		// starting the machine with a bean:
		Bean firstBean = this.beansArr[remainingBeanCount - 1];
		beansList.add(0, firstBean);
		inFlightBeans.add(firstBean);
		remainingBeanCount--;
	}
	...
	public boolean advanceStep() {
		// we dont any remaining and list is empty returns false
		if (inFlightBeans.isEmpty() && remainingBeanCount == 0) {
			return false;
		}
		// add a bean to the in flight array-> move all beans down 1 ypos.
		// if ypos = slot count-1 then increment slot array, remove those beans
		// then choose
		// then add a bean if there is any remaining.\
		for (int i = 0; i < beansList.size(); i++) {
			Bean curr = beansList.get(i); // current bean
			if (curr != null) {
				if (i == slotCount - 1) {
					slots[curr.getXPos()]++;
					beansList.remove(curr);
					inFlightBeans.remove(curr);
				} else {
					curr.choose(); // bean is moving left or right
				}
			}
		}
		if (remainingBeanCount > 0) {
			Bean b = beansArr[remainingBeanCount - 1]; // getting a new bean to add to the machine
			beansList.add(0, b); // adding it at the top of the machine
			inFlightBeans.add(b);
			remainingBeanCount--;
		} else {
			beansList.add(0, null);
		}
